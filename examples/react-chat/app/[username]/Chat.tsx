...
function ChatPage({ username }: { username: string }) {
    ...
    useEffect(() => {
        ...
        messageClient.getLatestChat(username)
            .then((latestChat) => {
                ...
                unsubscribe = messageClient.watchChat(username, (newMessages) => {
    ...
    const handleOnSendMessage = async () => {
        const sentMessage = await messageClient.sendMessage(username, inputMessage);
        setInputMessage("")
        setMessages((previousMessages) => {
    ...
    return <main className={styles.chat}>
        <div className={styles.topBar}>
            ...
            <TemporaryAccount address={account.address} />
        </div>
        {loading || error ? <div className={styles.messageInfo}>{loading ? "Loading..." : error}</div> : messages.length === 0 ? <div className={styles.messageInfo}>Send a message to @{username}</div> : <ul className={styles.messageList}>
            {messages.map((message) => <MessageBubble message={message} clientAddress={account.address} key={message.id} />)}
        </ul>}
